      ...
      "Statement" : [
        {
          ...
          "Resource" : "${aws_s3_bucket.static_web_bucket.arn}/*",  //allow access to all objects in the bucket
          "Condition" : {
            "StringEquals" : {
...
resource "aws_s3_object" "objects" {
  for_each = fileset("${path.module}/static-web-app", "**/*")
  bucket   = aws_s3_bucket.static_web_bucket.id
  key    = each.value
  source = "${path.module}/static-web-app/${each.value}"

  etag = filemd5("${path.module}/static-web-app/${each.value}")
  content_type = lookup(
    {
      "html" : "text/html",
      "css" : "text/css",
